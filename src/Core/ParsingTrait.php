    ...
    private function _bind($key, $val)
    {
        ...
        if (is_array($val)) {
            switch ($val[0]) {
                case 'eq':
                    $zwKey .= '_eq_0';
                    $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' = ' . $zwKey;
                ...
                case 'neq':
                    ...
                    foreach ($val[1] as $k => $v) {   //多个不等于
                        ...
                        $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' != ' . $zwKeyNeq;
                        $this->_condition_bind[$zwKeyNeq] = $v;
                    }
                    break;
                case 'like':
                    $zwKey .= '_0';
                    $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' like ' . $zwKey;
                    $this->_condition_bind[$zwKey] = $val[1];
                    break;
                case 'notLike':
                    $zwKey .= '_0';
                    $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' not like ' . $zwKey;
                    $this->_condition_bind[$zwKey] = $val[1];
                    break;
                case 'gt':
                    $zwKey .= '_0';
                ...
                case 'in':
                    ...
                    $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' in (' . $zwKeyIn . ')';
                    break;
                case 'notIn':
                    $zwKeyIn = '';
                    foreach ($val[1] as $k => $v) {
                    ...
                    $zwKeyIn = trim($zwKeyIn, ',');
                    $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' not in (' . $zwKeyIn . ')';
                    break;
                case 'between':
                    $zwKeyMin = $zwKey . '_between_min_0';
                    $zwKeyMax = $zwKey . '_between_max_0';
                    $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' <= ' . $zwKeyMax;
                    $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' >= ' . $zwKeyMin;
                ...
                case 'is':
                    $zwKeyIs = $zwKey . '_is_0';
                    if ($val[1] === null) {
                        $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' is NULL';
                    } else {
                        $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' is ' . $zwKeyIs;
                ...
                case 'isNot':
                    ...
                    if ($val[1] === null) {
                        $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' is not NULL';
                    } else {
                        $this->_condition_str .= FastEngine::CONDITION_AND . $key . ' is not ' . $zwKeyIs;
                        $this->_condition_bind[$zwKeyIs] = $val[1];
                    }
                    break;
                case 'findInSet':
                    $zwKeyIs = $zwKey . '_findInSet_0';
                    $this->_condition_str .= FastEngine::CONDITION_AND . 'FIND_IN_SET(' . $zwKeyIs . ',' . $key . ')';
                    $this->_condition_bind[$zwKeyIs] = $val[1];
                    break;
                case 'more':
